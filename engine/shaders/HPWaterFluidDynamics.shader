...
void main() {
    ...
    float left = ReadHeight(coord + ivec2(-1, 0), size);
    float right = ReadHeight(coord + ivec2(1, 0), size);
    float down = ReadHeight(coord + ivec2(0, -1), size);
    float up = ReadHeight(coord + ivec2(0, 1), size);
    ...
    float inertia = (current - previous) * (1.0 - damping) * edgeRetention;
