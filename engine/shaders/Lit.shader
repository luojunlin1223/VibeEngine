...
uniform int   u_PointLightShadowIndex[MAX_POINT_LIGHTS]; // -1 = no shadow
...
uniform int   u_SpotLightShadowIndex[MAX_SPOT_LIGHTS]; // -1 = no shadow
...
// Point light shadow maps (max 2, cube maps with linear depth)
...
float PointShadowCalculation(int shadowIdx, vec3 fragPos, vec3 lightPos) {
    ...
    // Simple PCF with offset samples
