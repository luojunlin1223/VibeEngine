...
void main() {
    ...
    float strands = smoothstep(0.42, 0.95, interference);

    float slopeFocus = smoothstep(0.025, 0.45, length(N.xz));
    float foamOcclusion = 1.0 - clamp(absorptionFoam.a, 0.0, 1.0) * 0.65;
    float energy = strands * slopeFocus * sunFacing * depthFade * foamOcclusion;
    energy *= clamp(u_CausticStrength, 0.0, 8.0) * max(u_LightIntensity, 0.0);
    ...
    vec3 caustic = max(u_LightColor, vec3(0.0)) * waterTint * attenuation * energy;
