...
void Scene::OnRenderDeferred(const glm::mat4& viewProjection,
    ...
    m_RenderDiagnostics.HPWaterSSRTemporalResolveEnabled =
    ...
    m_RenderDiagnostics.HPWaterVolumeUpsampleGatherParityEnabled =
    ...
    hpWaterPointLightTopInfluenceScore = hpWaterPointCandidates.empty() ? 0.0f : hpWaterPointCandidates.front().Score;
    ...
    for (int i = 0; i < hpWaterNumPointLights; ++i) {
        ...
        hpWaterPointLightSelectedInfluenceSum += candidate.Score;
    }
    if (hpWaterPointCandidates.size() > static_cast<size_t>(MAX_POINT_LIGHTS)) {
        hpWaterPunctualPointLightsCapacitySkipped =
            static_cast<uint32_t>(hpWaterPointCandidates.size() - static_cast<size_t>(MAX_POINT_LIGHTS));
        hpWaterPunctualLightsCapacitySkipped += hpWaterPunctualPointLightsCapacitySkipped;
    }
    ...
    hpWaterSpotLightTopInfluenceScore = hpWaterSpotCandidates.empty() ? 0.0f : hpWaterSpotCandidates.front().Score;
    ...
    for (int i = 0; i < hpWaterNumSpotLights; ++i) {
        ...
        hpWaterSpotLightSelectedInfluenceSum += candidate.Score;
    }
    if (hpWaterSpotCandidates.size() > static_cast<size_t>(MAX_SPOT_LIGHTS)) {
        hpWaterPunctualSpotLightsCapacitySkipped =
            static_cast<uint32_t>(hpWaterSpotCandidates.size() - static_cast<size_t>(MAX_SPOT_LIGHTS));
        hpWaterPunctualLightsCapacitySkipped += hpWaterPunctualSpotLightsCapacitySkipped;
    }
    ...
    std::vector<HPWaterAreaLightCandidate> hpWaterAreaCandidates;
    std::array<glm::vec3, MAX_AREA_LIGHTS> hpWaterAreaPositions = {};
    std::array<glm::vec3, MAX_AREA_LIGHTS> hpWaterAreaRights = {};
    std::array<glm::vec3, MAX_AREA_LIGHTS> hpWaterAreaUps = {};
    std::array<glm::vec3, MAX_AREA_LIGHTS> hpWaterAreaForwards = {};
    std::array<glm::vec3, MAX_AREA_LIGHTS> hpWaterAreaColors = {};
    std::array<float, MAX_AREA_LIGHTS> hpWaterAreaIntensities = {};
    std::array<float, MAX_AREA_LIGHTS> hpWaterAreaRanges = {};
    std::array<float, MAX_AREA_LIGHTS> hpWaterAreaWidths = {};
    std::array<float, MAX_AREA_LIGHTS> hpWaterAreaHeights = {};
    {
        auto areaView = m_Registry.view<TransformComponent, AreaLightComponent>();
        for (auto areaEntity : areaView) {
            if (!IsEntityActiveInHierarchy(areaEntity))
                continue;
            ...
            glm::mat4 worldMat = GetWorldTransform(areaEntity);
            const glm::vec3 position = glm::vec3(worldMat[3]);
            const glm::vec3 right = glm::length(glm::vec3(worldMat[0])) > 0.0001f
                ? glm::normalize(glm::vec3(worldMat[0]))
                : glm::vec3(1.0f, 0.0f, 0.0f);
            const glm::vec3 up = glm::length(glm::vec3(worldMat[1])) > 0.0001f
                ? glm::normalize(glm::vec3(worldMat[1]))
                : glm::vec3(0.0f, 1.0f, 0.0f);
            const glm::vec3 forward = glm::length(glm::vec3(worldMat[2])) > 0.0001f
                ? glm::normalize(glm::vec3(worldMat[2]))
                : glm::vec3(0.0f, 0.0f, 1.0f);
            const glm::vec3 color(area.Color[0], area.Color[1], area.Color[2]);
            const float width = std::max(area.Width, 0.001f);
            const float height = std::max(area.Height, 0.001f);
            const float areaBoost = std::sqrt(width * height);
            hpWaterAreaCandidates.push_back({
                position,
                right,
                up,
                forward,
                ...
                width,
                height,
                hpWaterPointLightInfluenceScore(position, color, area.Intensity * areaBoost, area.Range)
            });
        }
    }
    std::sort(hpWaterAreaCandidates.begin(), hpWaterAreaCandidates.end(),
              [](const HPWaterAreaLightCandidate& a, const HPWaterAreaLightCandidate& b) {
                  return a.Score > b.Score;
              });
    hpWaterAreaLightCandidates = static_cast<uint32_t>(hpWaterAreaCandidates.size());
    hpWaterAreaLightTopInfluenceScore = hpWaterAreaCandidates.empty() ? 0.0f : hpWaterAreaCandidates.front().Score;
    hpWaterNumAreaLights = std::min(static_cast<int>(hpWaterAreaCandidates.size()), MAX_AREA_LIGHTS);
    for (int i = 0; i < hpWaterNumAreaLights; ++i) {
        const auto& candidate = hpWaterAreaCandidates[static_cast<size_t>(i)];
        hpWaterAreaPositions[i] = candidate.Position;
        hpWaterAreaRights[i] = candidate.Right;
        hpWaterAreaUps[i] = candidate.Up;
        hpWaterAreaForwards[i] = candidate.Forward;
        hpWaterAreaColors[i] = candidate.Color;
        hpWaterAreaIntensities[i] = candidate.Intensity;
        hpWaterAreaRanges[i] = candidate.Range;
        hpWaterAreaWidths[i] = candidate.Width;
        hpWaterAreaHeights[i] = candidate.Height;
        hpWaterAreaLightSelectedInfluenceSum += candidate.Score;
    }
        ...
        if (hasProjectedCorner) {
            ...
            if (maxScreenX >= minScreenX && maxScreenY >= minScreenY) {
                    ...
                    for (uint32_t tileX = minTileX; tileX <= maxTileX; ++tileX) {
                        ...
                        for (uint32_t i = 0; i < hpWaterLightPayloadAreaCount; ++i) {
                            ...
                            if (lightSphereOverlapsTile(candidate.Position,
                                                        ...
                                                        tileY)) {
                                ++tileAreaCount;
                                hpWaterTiledLightListReferences.push_back(0x80000000u | static_cast<uint32_t>(i));
                            }
                        }
                ...
                hpWaterTiledLightListLightReferenceCount =
                    static_cast<uint32_t>(hpWaterTiledLightListReferences.size());
                ...
                hpWaterTiledLightListTileHeaders.reserve(tileLightOffsets.size() * 2u);
                for (size_t tileIndex = 0; tileIndex < tileLightOffsets.size(); ++tileIndex) {
                    hpWaterTiledLightListTileHeaders.push_back(tileLightOffsets[tileIndex]);
                    hpWaterTiledLightListTileHeaders.push_back(tileLightCounts[tileIndex]);
                }
                for (uint32_t reference : hpWaterTiledLightListReferences) {
    ...
    for (auto entityID : view) {
        ...
        ++hpWaterFluidCaptureMeshCandidates;
        ...
        // ── Frustum cull ──
        {
            ...
            if (!isHPWater && !isWaterLayerForFluid && !isTransparentForFluid) {
                ...
                ++hpWaterFluidSceneWaterLayerSkipped;
    ...
    if (hpWaterFluidEnabled) {
        ...
        m_RenderDiagnostics.HPWaterFluidCaptureMeshCandidates = hpWaterFluidCaptureMeshCandidates;
        m_RenderDiagnostics.HPWaterFluidWaterLayerCandidates = hpWaterFluidWaterLayerCandidates;
        m_RenderDiagnostics.HPWaterFluidSceneOpaqueCandidates = hpWaterFluidSceneOpaqueCandidates;
        ...
        m_RenderDiagnostics.HPWaterFluidSceneWaterLayerSkipped = hpWaterFluidSceneWaterLayerSkipped;
    ...
    if (m_RenderDiagnostics.HPWaterGBufferDrawn > 0) {
        ...
        m_RenderDiagnostics.HPWaterAreaLightCandidates =
            hpWaterAreaLightCandidates;
        ...
        m_RenderDiagnostics.HPWaterAreaLightCapacity =
            static_cast<uint32_t>(MAX_AREA_LIGHTS);
        m_RenderDiagnostics.HPWaterLightSelectionBoundsValid = hpWaterLightSelectionBoundsValid;
        m_RenderDiagnostics.HPWaterLightSelectionCenter = hpWaterLightSelectionCenter;
        m_RenderDiagnostics.HPWaterLightSelectionRadius = hpWaterLightSelectionRadius;
        m_RenderDiagnostics.HPWaterPointLightTopInfluenceScore = hpWaterPointLightTopInfluenceScore;
        m_RenderDiagnostics.HPWaterSpotLightTopInfluenceScore = hpWaterSpotLightTopInfluenceScore;
        m_RenderDiagnostics.HPWaterAreaLightTopInfluenceScore = hpWaterAreaLightTopInfluenceScore;
        m_RenderDiagnostics.HPWaterPointLightSelectedInfluenceSum = hpWaterPointLightSelectedInfluenceSum;
        m_RenderDiagnostics.HPWaterSpotLightSelectedInfluenceSum = hpWaterSpotLightSelectedInfluenceSum;
        m_RenderDiagnostics.HPWaterAreaLightSelectedInfluenceSum = hpWaterAreaLightSelectedInfluenceSum;
        ...
        m_RenderDiagnostics.HPWaterPunctualPointLightsCapacitySkipped =
            hpWaterPunctualPointLightsCapacitySkipped;
        m_RenderDiagnostics.HPWaterPunctualSpotLightsCapacitySkipped =
            hpWaterPunctualSpotLightsCapacitySkipped;
        ...
        m_RenderDiagnostics.HPWaterTiledLightListGPUUploadEnabled =
            hpWaterTiledLightListGPUUploadEnabled &&
            m_DeferredRenderer.IsHPWaterTiledLightListGPUUploadValid();
        m_RenderDiagnostics.HPWaterTiledLightListGPUBuffer =
            m_DeferredRenderer.GetHPWaterTiledLightListBuffer();
        m_RenderDiagnostics.HPWaterTiledLightListGPUBufferBytes =
            m_DeferredRenderer.GetHPWaterTiledLightListGPUBufferBytes();
        m_RenderDiagnostics.HPWaterTiledLightListGPUHeaderBytes =
            m_DeferredRenderer.GetHPWaterTiledLightListGPUHeaderBytes();
        m_RenderDiagnostics.HPWaterTiledLightListGPUReferenceBytes =
        ...
        m_DeferredRenderer.CompositeHPWater(nearClip,
                                            ...
                                            hpWaterSpotInnerCos,
                                            hpWaterSpotOuterCos,
                                            hpWaterNumAreaLights,
                                            hpWaterAreaPositions,
                                            hpWaterAreaRights,
                                            hpWaterAreaUps,
                                            hpWaterAreaForwards,
                                            hpWaterAreaColors,
                                            hpWaterAreaIntensities,
                                            hpWaterAreaRanges,
                                            hpWaterAreaWidths,
                                            hpWaterAreaHeights,
                                            cameraView,
    ...
    m_RenderDiagnostics.HPWaterRefractionBufferFormatParityEnabled =
        m_DeferredRenderer.IsHPWaterRefractionBufferFormatParityEnabled();
