...
static void SerializeEntity(YAML::Emitter& out, Entity entity, entt::registry& registry) {
    ...
    if (entity.HasComponent<HPWaterComponent>()) {
        ...
        out << YAML::Key << "VolumeTemporalBlendFactor" << YAML::Value << w.VolumeTemporalBlendFactor;
        out << YAML::Key << "VolumeSpatialFilterEnabled" << YAML::Value << w.VolumeSpatialFilterEnabled;
        out << YAML::Key << "VolumeSpatialFilterIterations" << YAML::Value << w.VolumeSpatialFilterIterations;
        out << YAML::Key << "VolumeMotionVectorsEnabled" << YAML::Value << w.VolumeMotionVectorsEnabled;
        out << YAML::Key << "VolumeMotionVectorVelocityScale" << YAML::Value << w.VolumeMotionVectorVelocityScale;
        out << YAML::Key << "VolumeTemporalDepthRejection" << YAML::Value << w.VolumeTemporalDepthRejection;
        out << YAML::Key << "VolumeTemporalDepthThreshold" << YAML::Value << w.VolumeTemporalDepthThreshold;
        out << YAML::Key << "VolumeSpatialDepthAware" << YAML::Value << w.VolumeSpatialDepthAware;
        out << YAML::Key << "VolumeSpatialDepthSensitivity" << YAML::Value << w.VolumeSpatialDepthSensitivity;
        ...
        out << YAML::Key << "CausticAtlasResolution" << YAML::Value << w.CausticAtlasResolution;
    ...
    for (auto entityNode : entities) {
        ...
        if (auto wNode = entityNode["HPWaterComponent"]) {
            try {
                ...
                if (wNode["VolumeTemporalBlendFactor"]) w.VolumeTemporalBlendFactor = wNode["VolumeTemporalBlendFactor"].as<float>();
                if (wNode["VolumeSpatialFilterEnabled"]) w.VolumeSpatialFilterEnabled = wNode["VolumeSpatialFilterEnabled"].as<bool>();
                if (wNode["VolumeSpatialFilterIterations"]) w.VolumeSpatialFilterIterations = wNode["VolumeSpatialFilterIterations"].as<int>();
                if (wNode["VolumeMotionVectorsEnabled"]) w.VolumeMotionVectorsEnabled = wNode["VolumeMotionVectorsEnabled"].as<bool>();
                if (wNode["VolumeMotionVectorVelocityScale"]) w.VolumeMotionVectorVelocityScale = wNode["VolumeMotionVectorVelocityScale"].as<float>();
                if (wNode["VolumeTemporalDepthRejection"]) w.VolumeTemporalDepthRejection = wNode["VolumeTemporalDepthRejection"].as<bool>();
                if (wNode["VolumeTemporalDepthThreshold"]) w.VolumeTemporalDepthThreshold = wNode["VolumeTemporalDepthThreshold"].as<float>();
                if (wNode["VolumeSpatialDepthAware"]) w.VolumeSpatialDepthAware = wNode["VolumeSpatialDepthAware"].as<bool>();
                if (wNode["VolumeSpatialDepthSensitivity"]) w.VolumeSpatialDepthSensitivity = wNode["VolumeSpatialDepthSensitivity"].as<float>();
                ...
                if (wNode["CausticAtlasResolution"]) w.CausticAtlasResolution = wNode["CausticAtlasResolution"].as<int>();
                ...
                w.ForwardScatterBlurDensity = std::clamp(w.ForwardScatterBlurDensity, 0.0f, 4.0f);
                w.MultiScatterScale = std::clamp(w.MultiScatterScale, 0.0f, 32.0f);
                ...
                w.VolumeTemporalBlendFactor = std::clamp(w.VolumeTemporalBlendFactor, 0.0f, 0.98f);
                w.VolumeSpatialFilterIterations = std::clamp(w.VolumeSpatialFilterIterations, 1, 3);
                w.VolumeMotionVectorVelocityScale = std::clamp(w.VolumeMotionVectorVelocityScale, 0.0f, 10.0f);
                w.VolumeTemporalDepthThreshold = std::clamp(w.VolumeTemporalDepthThreshold, 0.0001f, 10.0f);
                w.VolumeSpatialDepthSensitivity = std::clamp(w.VolumeSpatialDepthSensitivity, 0.0f, 1000.0f);
