...
VibeEngine now has a first GPU height-texture pipeline: a persistent R16F ping-pong wave equation pass, serialized component controls, Render Debugger/diagnostic export, and GBuffer sampling for fluid normals/foam. It also has the first HPWater-style obstacle-boundary input: normalized water-height and scene-height textures are uploaded for the ocean simulation volume, and the wave pass treats pixels as blocked when `sceneHeight > waterHeight`. These height fields are currently generated from Scene mesh world AABBs, while the older per-ocean R8 obstacle mask remains as a fallback and diagnostic resource. The primary update path now runs through `HPWaterFluidDynamics.comp`, so the fluid simulation no longer depends on the earlier fullscreen fragment pass except as a fallback. HPWater's true top-down virtual camera draw passes for water height and scene height remain pending; the existing CPU mesh/spectrum path is still kept as the geometry fallback.
...
5. GPU fluid dynamics
   ...
   - Pending: replace the CPU AABB-generated height fields with real HPWater-style top-down water height and scene height capture draw passes.
...
- Interactive GPU fluid impulses run through the compute backend (`HPWaterFluidComputeRan=1`) and produce a non-empty `render_diagnostics_hpwater_fluid_height.bmp`; overlapping non-water meshes produce valid `render_diagnostics_hpwater_fluid_obstacle.bmp`, `render_diagnostics_hpwater_fluid_water_height.bmp`, and `render_diagnostics_hpwater_fluid_scene_height.bmp`, with `HPWaterFluidHeightFieldValid=1`.
