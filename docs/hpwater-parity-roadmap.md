...
VibeEngine now has the dedicated three-target HPWater GBuffer and diagnostics. It also merges valid HPWater surface depth back into the main scene depth buffer after building the opaque-only refraction depth pyramid, so later screen-space passes can distinguish water surface depth without causing HPWater refraction to self-intersect. It also generates a dedicated full-resolution R8 HPWater mask texture from the HPWater depth buffer and uses it as the GPU-side pass-isolation equivalent of HPWater's stencil path. A real platform stencil path can still be added later if exact stencil parity is needed.
