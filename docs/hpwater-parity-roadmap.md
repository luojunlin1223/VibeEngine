...
- `HPWaterCausticIrradiance.comp` is the first OpenGL compute slice of HPWater caustics: it scatters refracted caustic energy from the HPWater GBuffer, explicit mask, scene depth, light parameters, the current light-space water atlas, and the active directional shadow-depth cascade array into four R32UI atomic irradiance textures. The compute pass reconstructs water world positions from the camera depth, selects the matching directional-light cascade from the current CSM split distances, projects the water point through the uploaded cascade VP matrix, samples the real 2x2 water atlas tile instead of synthesizing cascade tiles from screen UVs, performs a first finite-step refracted light ray march against the shadow-depth texture array, and in RGB caustic mode projects R/B receivers with HPWater-style per-channel eta offsets before camera-space atomic scatter.
...
- `HPWaterFluidHeightCapture.shader` captures the first top-down R16F water-height and scene-height inputs used by fluid dynamics. The water target now rasterizes the displaced HPWater surface height from the actual water draw, while the scene target rasterizes opaque mesh footprints with per-object top height, using max blending to form an obstacle height field. The CPU fallback also samples the current HPWater dynamic mesh/spectrum height instead of only uploading a flat ocean height.
...
VibeEngine now has the first half-resolution `HPWaterVolume.shader` accumulation target with in-scattering, transmittance, and refracted depth, plus a temporal reprojection/history pass, a three-iteration depth-aware a-trous-style spatial filter, and a full-resolution joint-bilateral upsample pass. The temporal path now generates and consumes an explicit RG16F volume motion-vector texture, rejects mismatched history with both center-depth and neighborhood-depth tests, and clamps history color/transmittance to the current 3x3 neighborhood before blending. True parity with HPWater/HDRP's full scene/object motion-vector source and all validation heuristics remains pending.
...
VibeEngine now has the first caustic resource slices: `HPWaterCaustic.shader` writes a full-resolution RGBA16F caustic texture, supports a screen-space single-channel/RGB dispersion mode, consumes a compute-generated RGBA16F irradiance texture, `HPWaterCausticFilter.shader` performs a two-pass edge-aware denoise/filter, `HPWaterComposite.shader` consumes the filtered texture when available, `HPWaterVolume.shader` injects filtered caustic energy into volume scattering, and `HPWaterCausticAtlas.shader` captures water-only payloads from all four directional-light cascade views into a 2x2 atlas. `HPWaterCausticIrradiance.comp` now uses HPWater-style multi-writer atomic irradiance accumulation through four R32UI channel textures, samples the actual light-space water atlas by reconstructing world position and applying the current CSM cascade VP matrices, binds the active CSM shadow-depth texture array for a first HPWater-style finite-step refracted light ray march, and uses HPWater-style R/G/B eta offsets to project per-channel RGB caustic receivers before camera-space scatter. `HPWaterCausticIrradianceResolve.comp` resolves those buffers into RGB caustic irradiance for filtering/composite. Diagnostics export `render_diagnostics_hpwater_caustic.bmp`, `render_diagnostics_hpwater_caustic_compute_irradiance.bmp`, `render_diagnostics_hpwater_caustic_filtered.bmp`, and `render_diagnostics_hpwater_caustic_atlas.bmp`, and report whether the atlas, compute irradiance path, atomic accumulation path, shadow-depth cascade array, and RGB receiver projection path were consumed by accumulation. Remaining caustic parity work is exact HPWater atlas-space receiver output, tuned exponential light-space stepping/dither, and closer spectral/cascade weighting.
...
4. Volumetric pass
   ...
   - Pending: add full scene/object motion-vector parity and closer HPWater/HDRP validation heuristics.
...
5. GPU fluid dynamics
   ...
   - Pending: replace the remaining per-object scene top-height approximation with stricter HPWater layer/filtering parity and validate GPU FFT source/capture-space conventions.
...
6. Caustics
   ...
   - Pending: replace the transitional camera-space receiver scatter with exact HPWater atlas-space receiver output, add tuned exponential stepping/dither parity and closer spectral/cascade weighting.
...
- Interactive GPU fluid impulses run through the compute backend (`HPWaterFluidComputeRan=1`) and produce a non-empty `render_diagnostics_hpwater_fluid_height.bmp`; overlapping non-water meshes produce valid `render_diagnostics_hpwater_fluid_obstacle.bmp`, `render_diagnostics_hpwater_fluid_water_height.bmp`, and `render_diagnostics_hpwater_fluid_scene_height.bmp`, with `HPWaterFluidHeightCaptureRan=1`, `HPWaterFluidHeightCaptureValid=1`, `HPWaterFluidDisplacedWaterHeightCapture=1`, and `HPWaterFluidHeightFieldValid=1`.
