...
- `HPWaterCausticIrradiance.comp` is the first OpenGL compute slice of HPWater caustics: it scatters refracted caustic energy from the HPWater GBuffer, explicit mask, scene depth, light parameters, and current light-space water atlas into four R32UI atomic irradiance textures.
