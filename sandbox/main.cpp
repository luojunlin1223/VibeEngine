...
private:
    ...
    void DrawRenderDebuggerPanel() {
        ...
        ImGui::Text("HPWater: entities=%u mesh=%u queued=%u drawn=%u culled=%u",
            ...
            d.HPWaterDrawn,
            d.HPWaterCulled);
        ImGui::Text("HPWater pass: gbufferDrawn=%u composite=%d depthMerge=%d mask=%d hiz=%d/%u volume=%d temporal=%d history=%d filter=%d/%u upsample=%d compositeTex=%u refractWorld=%u refractMeta=%u",
        ...
        ImGui::Text("HPWater volume temporal: clamp=%d motion=%d explicitMV=%d strength=%.2f",
        ...
        ImGui::Text("HPWater light loop: valid=%d surfaceShadow=%d cascadeDither=%d punctual=%d point=%u spot=%u indirectScatter=%d bsdfWeights=%d skyRefl=%.3f indirect=%.3f dir=%.3f",
